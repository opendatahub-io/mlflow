...
- **P0 (Critical)**: Core security, basic RBAC, authentication, core tracking operations - 20 tests
- **P1 (High)**: Multi-workspace scenarios, admin access, resource-level permissions, advanced tracking - 31 tests
- **P2 (Medium)**: Caching, configuration, proxies, client integration, nested runs - 18 tests
- **P3 (Low)**: Non-Kubernetes deployment, edge cases, migration - 10 tests
...
| Test ID   | Test Name                                      | Priority | Complexity | Impact | Description                                                      | Prerequisites                            | Test Steps                                                                                                                                                                    | Expected Result                           | Automated | Automation Type |
| --------- | ---------------------------------------------- | -------- | ---------- | ------ | ---------------------------------------------------------------- | ---------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------- | --------- | --------------- |
| TC-WS-001 | Basic Workspace Access                         | P0       | Low        | High   | Verify user can only access workspaces they have permissions for | User with permission to namespace-A only | 1. Authenticate with service account token for namespace-A<br>2. Set X-MLFLOW-WORKSPACE header to namespace-A<br>3. Call GET /api/2.0/mlflow/experiments/list                 | Returns experiments from namespace-A only | No        | E2E             |
...
| Test ID     | Test Name                            | Priority | Complexity | Impact | Description                                            | Prerequisites                                   | Test Steps                                                                                                                                               | Expected Result                   | Automated | Automation Type |
| ----------- | ------------------------------------ | -------- | ---------- | ------ | ------------------------------------------------------ | ----------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------- | --------- | --------------- |
| TC-AUTH-001 | Service Account Token Authentication | P0       | Low        | High   | Verify authentication with valid service account token | Valid Kubernetes service account in namespace-A | 1. Generate token: `kubectl -n namespace-A create token mlflow-user`<br>2. Send request with Authorization: Bearer &lt;token&gt;<br>3. Access MLflow API | Request succeeds with valid token | No        | E2E             |
| TC-AUTH-002 | Invalid Token Rejection              | P0       | Low        | High   | Verify invalid/expired tokens are rejected             | None                                            | 1. Use expired or invalid token<br>2. Send request to MLflow API                                                                                         | 401 Unauthorized error            | No        | E2E             |
...
| TC-RBAC-002 | Write Permission on Experiments | P0       | Low        | High   | Verify user with create permission can create experiments       | Role with create verb on experiments resource                                            | 1. Create role with create verb on experiments resource<br>2. Bind role to user for namespace-A<br>3. Call POST /api/2.0/mlflow/experiments/create | Experiment created successfully | No        | E2E             |
| TC-RBAC-003 | Deny Write Without Permission   | P0       | Low        | High   | Verify user with only read permission cannot create experiments | User has only get/list verbs (no create)                                                 | 1. User has only get/list verbs (no create)<br>2. Call POST /api/2.0/mlflow/experiments/create                                                     | 403 Forbidden error             | No        | E2E             |

### Artifact Operations

| Test ID    | Test Name                                               | Priority | Complexity | Impact | Description                                                                                   | Prerequisites                                                                        | Test Steps                                                                                                                                                                                                                                 | Expected Result                                                                                             | Automated | Automation Type |
| ---------- | ------------------------------------------------------- | -------- | ---------- | ------ | --------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------- | --------- | --------------- |
| TC-EXP-001 | Create Experiment with Auto-Generated Artifact Location | P0       | Low        | High   | Verify artifact location is automatically prefixed with workspace                             | User with create permission on experiments                                           | 1. Set workspace to namespace-A<br>2. Call POST /api/2.0/mlflow/experiments/create with name="test-exp"<br>3. Verify artifact_location field                                                                                               | artifact_location = &lt;root&gt;/workspaces/namespace-A/&lt;exp-id&gt;                                      | No        | E2E             |
| TC-EXP-002 | Default Workspace Artifact Location                     | P3       | Low        | High   | Verify default workspace uses legacy root without workspace prefix                            | Default workspace exists                                                             | 1. Set workspace to "default"<br>2. Create experiment<br>3. Verify artifact_location                                                                                                                                                       | artifact_location = &lt;root&gt;/&lt;exp-id&gt; (no workspace prefix)                                       | No        | E2E             |
...
| TC-EXP-004 | Search Experiments Within Workspace                     | P0       | Low        | High   | Verify search results are workspace-scoped                                                    | Multiple experiments in multiple workspaces                                          | 1. Create exp-A in namespace-A<br>2. Create exp-B in namespace-B<br>3. Set workspace to namespace-A<br>4. Call GET /api/2.0/mlflow/experiments/search                                                                                      | Returns only exp-A                                                                                          | No        | E2E             |
| TC-EXP-005 | Search Experiments Across Multiple Workspaces           | P1       | Medium     | High   | Verify user with access to multiple workspaces can search experiments in different workspaces | User has access to namespace-A and namespace-B; Experiments exist in both workspaces | 1. Create exp-A in namespace-A<br>2. Create exp-B in namespace-B<br>3. Set workspace to namespace-A<br>4. Call GET /api/2.0/mlflow/experiments/search<br>5. Set workspace to namespace-B<br>6. Call GET /api/2.0/mlflow/experiments/search | First call returns only exp-A; Second call returns only exp-B                                               | No        | E2E             |
| TC-EXP-006 | Upload Artifact with Write Permission                   | P0       | Medium     | High   | Verify user with write permission can upload artifacts to workspace                           | User with write permission on namespace-A; Run exists in namespace-A                 | 1. Set workspace to namespace-A<br>2. Create run in experiment<br>3. Call POST /api/2.0/mlflow/artifacts/log with artifact file<br>4. Verify artifact uploaded successfully                                                                | Artifact uploaded and stored under workspace prefix                                                         | No        | E2E             |
| TC-EXP-007 | Upload Artifact with Custom Prefix                      | P1       | Medium     | Medium | Verify artifact upload with user-provided prefix uses that prefix in path                     | User with write permission on namespace-A; Run exists in namespace-A                 | 1. Set workspace to namespace-A<br>2. Create run in experiment<br>3. Call POST /api/2.0/mlflow/artifacts/log with artifact and prefix="models/v1"<br>4. Verify artifact path                                                               | Artifact stored at &lt;root&gt;/workspaces/namespace-A/&lt;exp-id&gt;/&lt;run-id&gt;/models/v1/&lt;file&gt; | No        | E2E             |
| TC-EXP-008 | Upload Artifact without Prefix Uses Default             | P1       | Medium     | Medium | Verify artifact upload without prefix appends workspace prefix automatically                  | User with write permission on namespace-A; Run exists in namespace-A                 | 1. Set workspace to namespace-A<br>2. Create run in experiment<br>3. Call POST /api/2.0/mlflow/artifacts/log with artifact file (no prefix)<br>4. Verify artifact path                                                                     | Artifact stored at &lt;root&gt;/workspaces/namespace-A/&lt;exp-id&gt;/&lt;run-id&gt;/&lt;file&gt;           | No        | E2E             |
| TC-EXP-009 | Deny Artifact Upload without Workspace Access           | P0       | Medium     | High   | Verify user cannot upload artifact to workspace without access                                | User with no permissions on namespace-B; Run exists in namespace-B                   | 1. User has no permissions on namespace-B<br>2. Set workspace to namespace-B<br>3. Attempt to call POST /api/2.0/mlflow/artifacts/log with artifact file                                                                                   | Returns 403 Forbidden error                                                                                 | No        | E2E             |

### Run Tracking Operations

| Test ID    | Test Name                                 | Priority | Complexity | Impact | Description                                          | Prerequisites                    | Test Steps                                                                                                                                                                | Expected Result                                      | Automated | Automation Type |
| ---------- | ----------------------------------------- | -------- | ---------- | ------ | ---------------------------------------------------- | -------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------- | --------- | --------------- |
| TC-RUN-001 | Create Run in Workspace-Scoped Experiment | P0       | Low        | High   | Verify run inherits workspace from parent experiment | Experiment exists in namespace-A | 1. Set workspace to namespace-A<br>2. Call POST /api/2.0/mlflow/runs/create with experiment_id<br>3. Verify run is created with workspace=namespace-A                     | Run created with workspace inherited from experiment | No        | E2E             |
...
| TC-RUN-004 | Update Run Status                         | P0       | Low        | High   | Verify status transitions are workspace-aware        | Active run exists in namespace-A | 1. Set workspace to namespace-A<br>2. Call POST /api/2.0/mlflow/runs/update with run_id, status="FINISHED"<br>3. Verify status updated                                    | Run status updated to FINISHED                       | No        | E2E             |

### Artifact Logging and Retrieval

| Test ID    | Test Name                                  | Priority | Complexity | Impact | Description                                                  | Prerequisites                         | Test Steps                                                                                                                  | Expected Result                                               | Automated | Automation Type |
| ---------- | ------------------------------------------ | -------- | ---------- | ------ | ------------------------------------------------------------ | ------------------------------------- | --------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------- | --------- | --------------- |
| TC-ART-001 | Log Artifact to Workspace-Scoped Run       | P0       | Medium     | High   | Verify artifact is stored under workspace prefix             | Run exists in namespace-A             | 1. Set workspace to namespace-A<br>2. Log artifact file to run<br>3. Verify artifact URI contains /workspaces/namespace-A/  | Artifact stored under /workspaces/namespace-A/&lt;exp-id&gt;/ | No        | E2E             |
| TC-ART-002 | Retrieve Artifact from Run                 | P0       | Medium     | High   | Verify artifact retrieval is workspace-scoped                | Artifact exists in run in namespace-A | 1. Set workspace to namespace-A<br>2. Call GET /api/2.0/mlflow/artifacts/list with run_id<br>3. Download artifact           | Artifact retrieved successfully                               | No        | E2E             |
| TC-ART-003 | Cross-Workspace Artifact Access Prevention | P0       | Medium     | High   | Verify artifacts cannot be accessed from different workspace | Artifact exists in namespace-A run    | 1. Create run with artifact in namespace-A<br>2. Set workspace to namespace-B<br>3. Attempt to access artifact using run_id | Returns RESOURCE_DOES_NOT_EXIST or PERMISSION_DENIED          | No        | E2E             |
...
| TC-RES-003 | Jobs - Permission Enforcement                    | P1       | Medium     | Medium | User needs jobs permissions to access job APIs                         | User initially has no permissions on jobs resource         | 1. User has no permissions on jobs resource<br>2. Call job-related API<br>3. Grant get/list on jobs resource<br>4. Retry call         | First call fails with 403, second succeeds                      | No        | E2E             |
| TC-RES-004 | Prompts Share Permissions with Registered Models | P1       | Medium     | Medium | Verify prompts inherit permissions from registeredmodels resource      | User has registeredmodels permissions                      | 1. Grant permissions on registeredmodels resource<br>2. Call prompt-related APIs                                                      | Prompt operations allowed based on registeredmodels permissions | No        | E2E             |
| TC-RES-005 | Workspaces - List Permission                     | P1       | Low        | Medium | User with list permission on workspaces can list accessible namespaces | User has list permission on workspaces resource            | 1. Grant list on workspaces resource<br>2. Call GET /api/2.0/mlflow/workspaces/list                                                   | Returns filtered workspace list                                 | No        | E2E             |

### Advanced Run Operations

| Test ID    | Test Name                             | Priority | Complexity | Impact | Description                                             | Prerequisites                     | Test Steps                                                                                                                                                        | Expected Result                             | Automated | Automation Type |
| ---------- | ------------------------------------- | -------- | ---------- | ------ | ------------------------------------------------------- | --------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------- | --------- | --------------- |
| TC-RUN-005 | Cross-Workspace Run Access Prevention | P1       | Medium     | High   | Verify runs cannot be accessed from different workspace | Run exists in namespace-A         | 1. Create run in namespace-A<br>2. Set workspace to namespace-B<br>3. Attempt to access run using run_id                                                          | Returns RESOURCE_DOES_NOT_EXIST             | No        | E2E             |
| TC-RUN-006 | Search Runs Within Workspace          | P1       | Medium     | High   | Verify run search results are filtered by workspace     | Runs exist in multiple workspaces | 1. Create run-A in namespace-A<br>2. Create run-B in namespace-B<br>3. Set workspace to namespace-A<br>4. Call POST /api/2.0/mlflow/runs/search                   | Returns only run-A                          | No        | E2E             |
| TC-RUN-007 | Batch Log Metrics Workspace Isolation | P1       | Medium     | High   | Verify batch operations respect workspace boundaries    | Run exists in namespace-A         | 1. Set workspace to namespace-A<br>2. Call POST /api/2.0/mlflow/runs/log-batch with multiple metrics<br>3. Verify all metrics logged correctly                    | All metrics logged to correct workspace     | No        | E2E             |
...
| Test ID    | Test Name                                   | Priority | Complexity | Impact | Description                                              | Prerequisites                        | Test Steps                                                                                                                                                        | Expected Result                                            | Automated | Automation Type |
| ---------- | ------------------------------------------- | -------- | ---------- | ------ | -------------------------------------------------------- | ------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------- | --------- | --------------- |
| TC-ART-004 | Default Workspace Artifact Compatibility    | P1       | Medium     | High   | Verify legacy artifact URIs work in default workspace    | Default workspace configured         | 1. Set workspace to "default"<br>2. Create experiment<br>3. Create run and log artifact<br>4. Verify artifact URI has no workspace prefix                         | Artifact URI = &lt;root&gt;/&lt;exp-id&gt;/&lt;run-id&gt;/ | No        | E2E             |
| TC-ART-005 | Workspace-Specific Artifact Root Resolution | P1       | High       | High   | Verify WorkspaceProvider.resolve_artifact_root() is used | Custom workspace provider configured | 1. Configure provider with custom artifact root for namespace-A<br>2. Create experiment in namespace-A<br>3. Verify experiment artifact_location uses custom root | Artifact location uses provider-resolved root              | No        | E2E             |
...
| TC-MODEL-004 | Cross-Workspace Model Search Prevention | P1       | Medium     | High   | Verify cross-workspace model search is blocked          | Models exist in multiple workspaces    | 1. Create model-A in namespace-A<br>2. Create model-B in namespace-B<br>3. Set workspace to namespace-A<br>4. Call GET /api/2.0/mlflow/registered-models/search | Returns only model-A                             | No        | E2E             |
...
| TC-CACHE-002 | Cache Expiration               | P2       | High       | Medium | Cached decisions expire after TTL                        | MLflow server with short cache TTL         | 1. Set MLFLOW_K8S_AUTH_CACHE_TTL_SECONDS=5<br>2. Make API call<br>3. Wait 6 seconds<br>4. Make same API call                                                                      | Second call triggers new Kubernetes authorization check          | No        | E2E             |
| TC-CACHE-003 | Permission Change During Cache | P2       | High       | Medium | Verify behavior when permissions change during cache TTL | User with permissions that can be modified | 1. User has read permission, make API call<br>2. Revoke permission in Kubernetes<br>3. Immediately retry call (within cache TTL)<br>4. Wait for cache expiration<br>5. Retry call | Call succeeds during cache TTL, fails after expiration           | No        | E2E             |

### Workspace Configuration

| Test ID    | Test Name                             | Priority | Complexity | Impact | Description                                                                       | Prerequisites                                                       | Test Steps                                                                                                                                                                                                                                                         | Expected Result                                                         | Automated | Automation Type |
| ---------- | ------------------------------------- | -------- | ---------- | ------ | --------------------------------------------------------------------------------- | ------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------- | --------- | --------------- |
| TC-CFG-001 | Label Selector Filtering              | P2       | Medium     | Medium | Verify only labeled namespaces appear as workspaces                               | MLflow server configured with label selector                        | 1. Set MLFLOW_K8S_WORKSPACE_LABEL_SELECTOR=mlflow-enabled=true<br>2. Create namespace-A with label mlflow-enabled=true<br>3. Create namespace-B without label<br>4. List workspaces                                                                                | Only namespace-A appears in workspace list                              | No        | Integration     |
| TC-CFG-002 | Namespace Exclusion Globs             | P2       | Medium     | Low    | Custom namespace exclusion patterns work                                          | MLflow server with custom exclusion globs                           | 1. Set MLFLOW_K8S_NAMESPACE_EXCLUDE_GLOBS=test-\*,dev-\*<br>2. Create namespaces: test-1, dev-1, prod-1<br>3. List workspaces                                                                                                                                      | Only prod-1 appears (plus kube-\* and openshift-\* excluded by default) | No        | Integration     |
| TC-CFG-003 | Default Workspace                     | P2       | Low        | Low    | Default workspace used when none specified                                        | MLflow server configured with default workspace                     | 1. Set MLFLOW_K8S_DEFAULT_WORKSPACE=namespace-A<br>2. Make API call without X-MLFLOW-WORKSPACE header<br>3. Verify experiment created in namespace-A                                                                                                               | Request uses namespace-A context                                        | No        | Integration     |
| TC-CFG-004 | Workspace Description from Annotation | P2       | Low        | Low    | Workspace descriptions populated from namespace annotations                       | Namespace with mlflow.kubeflow.org/workspace-description annotation | 1. Create namespace with annotation: mlflow.kubeflow.org/workspace-description="Team A workspace"<br>2. List workspaces<br>3. Verify description field                                                                                                             | Workspace includes description from annotation                          | No        | Integration     |
| TC-CFG-005 | Label Prefix List Configuration       | P1       | Medium     | High   | Verify admin can configure list of label prefixes to select namespaces            | Admin access to configure label selectors                           | 1. Configure label selector with multiple prefixes: mlflow-enabled,app-enabled<br>2. Create namespace-A with label mlflow-enabled=true<br>3. Create namespace-B with label app-enabled=true<br>4. Create namespace-C with no matching labels<br>5. List workspaces | Both namespace-A and namespace-B appear; namespace-C excluded           | No        | Integration     |
| TC-CFG-006 | Exclusion Priority Over Inclusion     | P1       | Medium     | High   | Verify excluded labels take priority over included labels when namespace has both | Admin access; Namespaces with mixed labels                          | 1. Configure label selector to include mlflow-enabled labels<br>2. Create namespace with both mlflow-enabled=true AND openshift-system=true labels<br>3. List workspaces<br>4. Verify namespace excluded despite having included label                             | Namespace with excluded label not available even with included label    | No        | Integration     |
| TC-CFG-007 | Default Exclusion Patterns            | P1       | Medium     | High   | Verify default exclusion of kube-_ and openshift-_ namespaces                     | Default MLflow configuration; Kubernetes/OpenShift cluster          | 1. Deploy MLflow with default configuration (no custom exclusions)<br>2. Verify kube-system, kube-public, kube-node-lease namespaces exist<br>3. Verify openshift-\* namespaces exist (on OpenShift)<br>4. List workspaces                                         | kube-_ and openshift-_ namespaces excluded from workspace list          | No        | Integration     |

### Proxy Authentication

| Test ID      | Test Name                       | Priority | Complexity | Impact | Description                                       | Prerequisites                        | Test Steps                                                                                                                          | Expected Result                               | Automated | Automation Type |
| ------------ | ------------------------------- | -------- | ---------- | ------ | ------------------------------------------------- | ------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------- | --------- | --------------- |
| TC-PROXY-001 | X-Forwarded-Access-Token Header | P2       | Medium     | Medium | Verify proxy can forward tokens via custom header | MLflow deployed behind reverse proxy | 1. Deploy MLflow behind reverse proxy<br>2. Configure proxy to set X-Forwarded-Access-Token header<br>3. Make request through proxy | Authentication succeeds using forwarded token | No        | E2E             |

### Client Integration

| Test ID       | Test Name                                    | Priority | Complexity | Impact | Description                                   | Prerequisites                  | Test Steps                                                                                                                     | Expected Result                                  | Automated | Automation Type |
| ------------- | -------------------------------------------- | -------- | ---------- | ------ | --------------------------------------------- | ------------------------------ | ------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------ | --------- | --------------- |
| TC-CLIENT-001 | Python Client - set_workspace()              | P2       | Low        | Medium | Verify Python SDK workspace setting           | MLflow Python client installed | 1. Run: `import mlflow`<br>2. Run: `mlflow.set_workspace("namespace-A")`<br>3. Run: `mlflow.create_experiment("test")`         | Experiment created in namespace-A                | Yes       | Unit            |
| TC-CLIENT-002 | Environment Variable - MLFLOW_WORKSPACE      | P2       | Low        | Medium | Verify workspace set via environment variable | MLflow CLI installed           | 1. Run: `export MLFLOW_WORKSPACE=namespace-A`<br>2. Run: `mlflow experiments create --experiment-name test`                    | Experiment created in namespace-A                | Yes       | Unit            |
| TC-CLIENT-003 | Environment Variable - MLFLOW_TRACKING_TOKEN | P2       | Low        | Medium | Verify token set via environment variable     | MLflow CLI installed           | 1. Run: `export MLFLOW_TRACKING_TOKEN=$(kubectl -n namespace-A create token mlflow-user)`<br>2. Run: `mlflow experiments list` | Authentication succeeds using token from env var | Yes       | Unit            |

### System Namespace Filtering

| Test ID    | Test Name                           | Priority | Complexity | Impact | Description                                              | Prerequisites                               | Test Steps                                                                                              | Expected Result                                         | Automated | Automation Type |
| ---------- | ----------------------------------- | -------- | ---------- | ------ | -------------------------------------------------------- | ------------------------------------------- | ------------------------------------------------------------------------------------------------------- | ------------------------------------------------------- | --------- | --------------- |
| TC-SYS-001 | Kubernetes System Namespaces Hidden | P2       | Low        | Low    | System namespaces filtered from workspace list           | Cluster admin access                        | 1. List workspaces as cluster admin<br>2. Verify kube-system, kube-public, kube-node-lease not included | System namespaces (kube-\*) filtered out                | No        | Integration     |
| TC-SYS-002 | OpenShift System Namespaces Hidden  | P2       | Low        | Low    | OpenShift system namespaces filtered from workspace list | OpenShift cluster with cluster admin access | 1. On OpenShift cluster, list workspaces<br>2. Verify openshift-\* namespaces not included              | OpenShift system namespaces (openshift-\*) filtered out | No        | Integration     |

### Nested Runs and Run Relationships

| Test ID       | Test Name                              | Priority | Complexity | Impact | Description                                         | Prerequisites                    | Test Steps                                                                                                                                              | Expected Result                                      | Automated | Automation Type |
| ------------- | -------------------------------------- | -------- | ---------- | ------ | --------------------------------------------------- | -------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------- | --------- | --------------- |
| TC-NESTED-001 | Create Nested Run Within Workspace     | P2       | Medium     | Medium | Verify child run inherits workspace from parent run | Parent run exists in namespace-A | 1. Set workspace to namespace-A<br>2. Create parent run<br>3. Create child run with parent_run_id<br>4. Verify child run inherits workspace from parent | Child run has same workspace as parent run           | No        | E2E             |
...
| Test ID   | Test Name                                    | Priority | Complexity | Impact | Description                                                          | Prerequisites                                          | Test Steps                                                                                                                                        | Expected Result                                                     | Automated | Automation Type |
| --------- | -------------------------------------------- | -------- | ---------- | ------ | -------------------------------------------------------------------- | ------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------- | --------- | --------------- |
| TC-NK-001 | MLflow Without Kubernetes Workspace Provider | P3       | Low        | Low    | MLflow works without workspace provider enabled                      | MLflow deployed without workspace provider             | 1. Deploy MLflow without --app-name kubernetes-auth<br>2. Do not configure workspace-store-uri<br>3. Create experiment                            | MLflow operates in single-tenant mode (no workspace isolation)      | No        | E2E             |
| TC-NK-002 | Workspace APIs Unavailable Without Provider  | P3       | Low        | Low    | Workspace endpoints return appropriate errors when provider disabled | MLflow deployed without workspace provider             | 1. Deploy MLflow without workspace provider<br>2. Call GET /api/2.0/mlflow/workspaces/list                                                        | 404 Not Found or appropriate error indicating feature not available | No        | E2E             |
| TC-NK-003 | Migration from Non-Kubernetes to Kubernetes  | P3       | High       | Low    | Existing data accessible after enabling workspace provider           | MLflow with existing experiments in non-workspace mode | 1. Create experiments in non-workspace mode<br>2. Enable workspace provider<br>3. Configure default workspace<br>4. Verify experiments accessible | Legacy experiments mapped to default workspace                      | No        | E2E             |

### Workspace Migration and Compatibility

| Test ID    | Test Name                  | Priority | Complexity | Impact | Description                                                 | Prerequisites                                          | Test Steps                                                                                                                                                            | Expected Result                                                      | Automated | Automation Type |
| ---------- | -------------------------- | -------- | ---------- | ------ | ----------------------------------------------------------- | ------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------- | --------- | --------------- |
| TC-MIG-001 | Backfill Verification      | P3       | Medium     | Low    | Verify legacy resources backfilled with workspace='default' | Existing MLflow deployment with data                   | 1. Record existing experiments/models before migration<br>2. Enable multi-tenancy mode<br>3. Run migration<br>4. Verify all legacy resources have workspace='default' | All legacy resources backfilled with workspace='default'             | No        | E2E             |
...
| Test ID     | Test Name                             | Priority | Complexity | Impact | Description                                                        | Prerequisites                             | Test Steps                                                                                                                                                                                    | Expected Result                                                | Automated | Automation Type |
| ----------- | ------------------------------------- | -------- | ---------- | ------ | ------------------------------------------------------------------ | ----------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------- | --------- | --------------- |
| TC-EDGE-001 | Very Long Namespace Names             | P3       | Low        | Low    | Verify handling of maximum length namespace names                  | Ability to create namespaces              | 1. Create namespace with 63-character name (Kubernetes limit)<br>2. Enable as MLflow workspace<br>3. Perform operations                                                                       | Works correctly with max-length namespace                      | No        | E2E             |
| TC-EDGE-002 | Special Characters in Workspace Names | P3       | Low        | Low    | Verify handling of valid Kubernetes special characters             | Ability to create namespaces              | 1. Create namespace with dashes, dots (valid K8s chars)<br>2. Test workspace access<br>3. Verify workspace name properly handled                                                              | Workspace name properly encoded/decoded in API calls           | No        | E2E             |
...
| -------------- | ---------- | -------------------------------------------------------------------------------- | ----------------------- |
| P0 - Critical  | 20         | Core security, authentication, basic RBAC, core tracking operations              | 1.5-3 hours             |
| P1 - High      | 31         | Multi-workspace access, admin scenarios, resource permissions, advanced tracking | 3.5-5 hours             |
| P2 - Medium    | 18         | Caching, configuration, proxies, client integration, nested runs                 | 2-3 hours               |
| P3 - Low       | 10         | Non-Kubernetes deployment, edge cases, migration                                 | 1.5-2 hours             |
| **Total**      | **79**     | **Complete multitenancy validation**                                             | **8.5-13 hours**        |
