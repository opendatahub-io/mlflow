...
def workspace_store(tmp_path, monkeypatch):
    ...
    # Provide a shim for MlflowException.invalid_state if it is not defined.
    if not hasattr(MlflowException, "invalid_state"):
        monkeypatch.setattr(
            MlflowException,
            "invalid_state",
            staticmethod(
                lambda message, **kwargs: MlflowException(
                    message, error_code=INVALID_STATE, **kwargs
                )
            ),
            raising=False,
        )
