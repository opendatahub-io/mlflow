...
def test_serving_artifacts_auto_scopes_workspace_paths(workspace_tracking_store, monkeypatch):
    ...
    def _unexpected_provider_call(*_args, **_kwargs):
        raise AssertionError("Workspace provider should not be consulted when serving artifacts")

    monkeypatch.setattr(
        SqlAlchemyStore, "_resolve_workspace_artifact_root", _unexpected_provider_call
    )
