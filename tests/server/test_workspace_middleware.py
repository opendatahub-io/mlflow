...
def flask_workspace_app(monkeypatch):
    ...
    @app.route("/ping")
    def _ping():
        return workspace_context.get_current_workspace() or "none"
...
def _fastapi_workspace_app(monkeypatch):
    ...
    @app.get("/ajax-api/3.0/ping")
    async def ping():
        return {"workspace": workspace_context.get_current_workspace()}

    return app


def test_fastapi_workspace_middleware_sets_context(monkeypatch):
    app = _fastapi_workspace_app(monkeypatch)
    monkeypatch.setattr(
    ...
    resp = client.get("/ajax-api/3.0/ping", headers={WORKSPACE_HEADER_NAME: "team-fast"})
...
def test_fastapi_workspace_middleware_requires_header(monkeypatch):
    ...
    client = TestClient(app)
    resp = client.get("/ajax-api/3.0/ping")
    assert resp.status_code == 400
    assert "Active workspace is required" in resp.json()["message"]
    assert workspace_context.get_current_workspace() is None
