...
def test_create_registered_model_permission(store):
    ...
    duplicate_permission_pattern = (
        ...
        rf"\(workspace={DEFAULT_WORKSPACE_NAME}, name={name1}, username={username1}\) "
        "already exists"
    )
    with pytest.raises(
        MlflowException,
...
def test_set_workspace_permission_creates_and_updates(store):
    ...
    assert perm.workspace_name == workspace
...
def test_list_workspace_permissions(store):
    ...
    actual = {
        (perm.workspace_name, perm.username, perm.resource_type, perm.permission) for perm in perms
    }
    expected = {
        (p1.workspace_name, p1.username, p1.resource_type, p1.permission),
        (p2.workspace_name, p2.username, p2.resource_type, p2.permission),
    }
    assert actual == expected

    perms_other = store.list_workspace_permissions(other_workspace)
    assert {
        (perm.workspace_name, perm.username, perm.resource_type, perm.permission)
        for perm in perms_other
    } == {(p3.workspace_name, p3.username, p3.resource_type, p3.permission)}
...
def test_delete_workspace_permissions_for_workspace(store):
    ...
    assert remaining[0].workspace_name == other_workspace
