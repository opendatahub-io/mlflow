...
**Resource scoping**
: Each workspace keeps its own resources such as experiments, registered models, prompts, and artifacts. Workspace-level permissions control who can see or modify those resources. All MLflow client and API usage remains the same otherwise.
...
- Multiple teams sharing a single MLflow instance while keeping experiments, registered models, and prompts organized
...
- A user with workspace-level `READ` permission can access all experiments, registered models, and prompts in that workspace
