...
def _build_workspace_app(monkeypatch):
    ...
    monkeypatch.setattr(
        "kubernetes_workspace_provider.auth.KubernetesAuthorizer.is_allowed",
        lambda self, token, resource, verb, namespace: True,
    )
    monkeypatch.setattr(
        "kubernetes_workspace_provider.auth._load_kubernetes_configuration",
        lambda: None,
...
def test_forwarded_access_token_header_allows_request(monkeypatch):
    ...
    args = mock_is_allowed.call_args[0]
...
def test_permission_denied_returns_403(monkeypatch):
    ...
    mock_is_allowed.assert_called_once()
    _, resource, verb, namespace = mock_is_allowed.call_args[0]
...
def test_workspace_scope_string_is_normalized(monkeypatch):
    ...
    authorizer.is_allowed.assert_called_once_with("valid-token", "experiments", "list", "team-a")
...
def test_workspace_scope_falls_back_to_view_args(monkeypatch):
    ...
    authorizer.is_allowed.assert_called_once_with("scope-token", "workspaces", "get", "team-a")
...
def test_experiment_permissions_are_checked_first(monkeypatch):
    ...
    authorizer = KubernetesAuthorizer(cache_ttl_seconds=1)

    def _fake_permission(token, resource, verb, namespace):
    ...
    accessible = authorizer.accessible_workspaces("token", ["team-a", "team-b"])
