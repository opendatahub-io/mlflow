...
def _authorize_request(
    *,
    ...
    if rule.verb is not None:
        ...
        allowed = authorizer.is_allowed(token, resource_type, rule.verb, workspace_name)
        if not allowed:
            raise MlflowException(
    ...
    username = _parse_jwt_subject(token, username_claim)
    return _AuthorizationResult(token=token, rule=rule, username=username)


class KubernetesAuthorizer:
    def __init__(self, cache_ttl_seconds: float, group: str = DEFAULT_AUTH_GROUP) -> None:
        self._group = group
        self._base_configuration = _load_kubernetes_configuration()
        self._cache = _AuthorizationCache(cache_ttl_seconds)

    def _build_api_client(self, token: str) -> client.ApiClient:
    ...
    def _submit_self_subject_access_review(
        ...
        api_client = self._build_api_client(token)
        try:
    ...
    def is_allowed(
        ...
        cache_key = (sha256(token.encode("utf-8")).hexdigest(), namespace, resource, verb)
        ...
        try:
            allowed = self._submit_self_subject_access_review(token, resource, verb, namespace)
        ...
        _logger.debug(
            "SSAR evaluated token_hash=%s resource=%s namespace=%s verb=%s allowed=%s",
            token_hash,
            resource,
            namespace,
            verb,
    ...
    def accessible_workspaces(self, token: str, names: Iterable[str]) -> set[str]:
        accessible: set[str] = set()
        token_hash = sha256(token.encode("utf-8")).hexdigest()
        for workspace_name in names:
            for resource in _WORKSPACE_PERMISSION_RESOURCE_PRIORITY:
                if self.is_allowed(token, resource, "list", workspace_name):
                    accessible.add(workspace_name)
                    _logger.debug(
                        "Workspace %s included for token_hash=%s via resource=%s",
                        workspace_name,
                        token_hash,
                        resource,
                    )
                ...
                _logger.debug(
                    "Workspace %s excluded for token_hash=%s; no list permission detected",
                    workspace_name,
                    token_hash,
                )
        return accessible
...
class KubernetesAuthConfig:
    ...
    def from_env(cls) -> "KubernetesAuthConfig":
        ...
        return cls(cache_ttl_seconds=cache_ttl_seconds, username_claim=username_claim)
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        try:
            _authorize_request(
                ...
                username_claim=self.config_values.username_claim,
...
def create_app(app: Flask = mlflow_app) -> Flask:
    ...
    authorizer = KubernetesAuthorizer(cache_ttl_seconds=config_values.cache_ttl_seconds)
    ...
    def _k8s_auth_before_request():
        ...
        try:
            auth_result = _authorize_request(
                ...
                username_claim=config_values.username_claim,
        ...
        g.mlflow_k8s_token = auth_result.token
    ...
    def _k8s_auth_after_request(response: Response):
        try:
            ...
            token = getattr(g, "mlflow_k8s_token", None)
            can_filter_response = (
                response.mimetype == "application/json" and response.status_code < 400
            )
            if not (should_filter and token and can_filter_response):
                return response
            ...
            accessible = authorizer.accessible_workspaces(
                token, [name for name in workspace_names if isinstance(name, str)]
            )
            payload["workspaces"] = [
