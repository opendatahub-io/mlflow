...
class MlflowConfigCache:
    ...
    def __init__(
        ...
        ensure_artifact_connection_secret_cache: Callable[[], None] | None = None,
    ):
        self._api = api
...
class SecretCache:
    """Caches mlflow-artifact-connection secrets using a watch loop.

    Uses ``list_secret_for_all_namespaces`` with a ``metadata.name`` field
    selector.  On Kubernetes 1.27+ (KEP-2441), the API server passes the field
    selector to the authorizer, so a ClusterRole with
    ``resourceNames: ["mlflow-artifact-connection"]`` and ``verbs: ["list", "watch"]``
    is sufficient -- no broad secrets access is required.
    """
    ...
    def _try_reload(self) -> bool:
        _logger.debug("Secret cache unavailable; attempting immediate reload")
        self._refresh_full()
        self._retry_event.set()
        ...
        try:
            ...
            if exc.status == 403:
                _logger.warning(
                    ...
                    "Ensure RBAC allows 'list' and 'watch' on secrets with "
                    "resourceNames ['%s']. Per-namespace artifact root overrides "
