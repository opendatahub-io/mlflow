  ...
  return (
    <div css={{ flex: 1, padding: theme.spacing.md, overflow: 'auto' }}>
      ...
      >
        ...
        <Typography.Text bold>
          <FormattedMessage defaultMessage="Title:" description="MCP server version detail title label" />
        </Typography.Text>
        <span css={{ display: 'flex', alignItems: 'center', gap: theme.spacing.sm }}>
          <Typography.Text>{version.server_json?.title || displayName}</Typography.Text>
          {version.display_name && (
            <Typography.Text color="secondary" size="sm">
              <FormattedMessage defaultMessage="Display name" description="MCP server version display name indicator" />
            </Typography.Text>
          )}
        </span>
