...
describe('useMlflowTraces', () => {
  ...
  test('does not call search api if feature flag is not enabled', async () => {
    ...
    expect(fetchAPI).toHaveBeenCalledTimes(0); // should not be called
    expect(result.current.data).toHaveLength(1);
    expect(result.current.data[0].evaluationId).toBe('artifact-entry-1');
  });
  ...
  test('falls back to artifact data if search fails', async () => {
    ...
    // Search call fails
    jest.mocked(fetchAPI).mockRejectedValue(new Error('Search failed'));
  ...
  test('uses artifact data if any paginated call fails mid-way', async () => {
    ...
    jest
      ...
      .mockRejectedValueOnce(new Error('Search failed on page 2'));
...
>>>>>>> 097fbfa8a (Refactor fetch utilities to use fetchAPI consistently across traces handling)
