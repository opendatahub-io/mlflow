...
const toMlflowRequestId = (id: string) => (id.startsWith('tr-') ? id : `tr-${id}`);

const TraceDetailContent: React.FC<{ traceId: string }> = ({ traceId }) => {
  const { data, isLoading } = useGetTracesById([toMlflowRequestId(traceId)]);
  const trace = data?.[0];
  ...
  if (!trace) {
    return <ModelTraceExplorerGenericErrorState />;
  }

  return <ModelTraceExplorer modelTrace={trace} />;
};

const MlflowTraceDetailWrapper: React.FC<MlflowTraceDetailWrapperProps> = ({
  traceId,
  workspace,
}) => {
  useEffect(() => {
  ...
  const memoryRouterEntries = useMemo(() => [`/traces/${traceId}`], [traceId]);
