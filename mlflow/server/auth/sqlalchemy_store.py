...
class SqlAlchemyStore:
    ...
    def _get_user(session, username: str) -> SqlUser:
        try:
            ...
            raise MlflowException(
                f"User with username={username} found multiple times",
                INVALID_STATE,
            )
    ...
    def _get_experiment_permission(
        ...
        try:
            ...
            raise MlflowException(
                f"Experiment permission with experiment_id={experiment_id} "
                f"and username={username} found multiple times",
                INVALID_STATE,
            )
