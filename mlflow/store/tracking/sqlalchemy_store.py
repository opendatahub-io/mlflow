...
class SqlAlchemyStore(WorkspaceAwareMixin, AbstractStore):
    ...
    def _create_default_experiment(self, session, workspace_override: str | None = None):
        ...
        if self._workspaces_enabled():
            ...
            if workspace == DEFAULT_WORKSPACE_NAME:
                self._insert_default_experiment_with_zero_id(session, creation_time, workspace)
                return
        ...
        self._insert_default_experiment_with_zero_id(session, creation_time, DEFAULT_WORKSPACE_NAME)

    def _insert_default_experiment_with_zero_id(
        self, session, creation_time: int, workspace: str
    ) -> None:
        ...
        default_experiment = {
            ...
            SqlExperiment.workspace.name: workspace,
            SqlExperiment.artifact_location.name: str(self._get_artifact_location(0, workspace)),
        ...
        try:
            ...
            session.rollback()
            _logger.debug(
                "Default experiment already exists in workspace '%s'; "
    ...
    def _get_artifact_location(self, experiment_id, workspace: str):
        ...
        if self._workspaces_enabled():
            serving_artifacts = (
                os.environ.get("_MLFLOW_SERVER_SERVE_ARTIFACTS", "").lower() == "true"
            )
    ...
    def _search_experiments(
        ...
        with self._workspace_session() as (session, workspace):
            ...
            stmt = reduce(lambda s, f: s.join(f), non_attribute_filters, select(SqlExperiment))
            stmt = stmt.options(*self._get_eager_experiment_query_options())
            ...
            if self._workspaces_enabled():
                filters.append(SqlExperiment.workspace == workspace)
    ...
    def _log_model_metrics(
        ...
        with self.ManagedSessionMaker() as session:
            ...
            if experiment_id is not None:
                try:
                    provided_experiment_id_int = int(experiment_id)
                except (TypeError, ValueError) as exc:
                    raise MlflowException(
                        f"Invalid experiment_id '{experiment_id}'. Expected an integer.",
                        error_code=INVALID_PARAMETER_VALUE,
                    ) from exc
                if provided_experiment_id_int != run.experiment_id:
                    raise MlflowException(
                        f"experiment_id '{experiment_id}' does not match the run "
    ...
    def _get_model_inputs(
        ...
        """
        Return model inputs for the specified run.

        Assumes the caller has already validated that ``run_id`` belongs to the active workspace, so
        no additional workspace filtering is applied.
        """
        return [
    ...
    def _get_model_outputs(
        ...
        """
        Return model outputs for the specified run.
    ...
    def _get_model_outputs_bulk(
        ...
        Assumes the caller has already validated that every ``run_id`` belongs to the active
        workspace, so no additional workspace filtering is applied.
        """
        outputs = (
            session.query(SqlInput)
            .filter(
    ...
    def link_traces_to_run(self, trace_ids: list[str], run_id: str) -> None:
        ...
        with self._workspace_session() as (session, workspace):
            allowed_trace_ids = trace_ids
            if self._workspaces_enabled():
                # Ensure the run exists in the active workspace
                self._get_run(run_uuid=run_id, session=session)
                # Ensure all traces exist in the active workspace (skip extra joins in
                # single-tenant mode)
                trace_query = (
                    self._trace_query(session)
                ...
                allowed_trace_ids = [
                    tid
                    for (tid,) in (
                        trace_query.with_entities(SqlTraceInfo.request_id)
    ...
    def _build_trace_filter_subquery(self, session, experiment_ids: list[str], filter_string: str):
        ...
        if self._workspaces_enabled():
            stmt = stmt.join(
                SqlExperiment, SqlTraceInfo.experiment_id == SqlExperiment.experiment_id
            ).where(SqlExperiment.workspace == self._get_active_workspace())
    ...
    def log_spans(self, location: str, spans: list[Span], tracking_uri=None) -> list[Span]:
        ...
        with self.ManagedSessionMaker() as session:
            ...
            # Single update with workspace guard. We avoid joins because SQLAlchemy forbids bulk
            # updates on joined queries, so enforce workspace scope via EXISTS.
            update_query = session.query(SqlTraceInfo).filter(
                SqlTraceInfo.request_id == trace_id,
        ...
        with self.ManagedSessionMaker() as session:
            try:
                ...
                if exc.error_code == ErrorCode.Name(RESOURCE_DOES_NOT_EXIST):
                    _logger.debug(
                        "Evaluation dataset with id '%s' not found when deleting tag '%s'. "
                        "The dataset may have already been deleted.",
                        dataset_id,
                        key,
                    )
                    return
